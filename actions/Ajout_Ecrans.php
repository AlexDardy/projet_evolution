<?php
    ...
    if( empty($_POST['Marque']) ){
        header('Location: ../pages/formulaire_ajout_ecrans.php');
        exit;
    }
    else{
   ...
   $requete_last_id = $bdd->prepare('SELECT MAX(CAST(SUBSTRING(ID,3, LENGTH(ID)) AS SIGNED)) AS LAST_ID from ecrans');
   ...
   if ($new_id < 10) {
    ...
    $requete = $bdd->prepare('INSERT INTO Ecrans(ID, Marque, Modele, Date_Achat, Duree_Garantie, ID_Utilisateurs) VALUES(:ID, :Marque, :Modele, :Date_Achat, :Duree_Garantie, :ID_Utilisateurs)');
    ...
    // Redirection du visiteur vers la liste des Ecrans
    header('Location: ../pages/Liste_Ecrans.php');
    exit;
?>