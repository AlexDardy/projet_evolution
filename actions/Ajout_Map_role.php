<?php
    ...
    if( empty($_POST['Nom']) ){
        header('Location: ../pages/formulaire_ajout_map_role.php');
        exit;
    }
    else{
        // Sinon si $_POST['nom'] n'est pas vide, on met la valeur dans la variable $nom et on continue.
        $NumeroSerie = $_POST['Nom'];
    };
    

   //Génération d'un nouvel ID Ecran incrémenté
   $requete_last_id = $bdd->prepare('SELECT MAX(CAST(SUBSTRING(ID,3, LENGTH(ID)) AS SIGNED)) AS LAST_ID from map_role');
   ...
   if ($new_id < 10) {
    ...
    $requete = $bdd->prepare('INSERT INTO map_role(Nom) VALUES(:Nom)');
