<?php
    ...
    if( empty($_POST['Marque']) ){
        header('Location: ../pages/formulaire_ajout_telephones.php');
        exit;
    }
    else{
   ...
   $requete_last_id = $bdd->prepare('SELECT MAX(CAST(SUBSTRING(ID,2, LENGTH(ID)) AS SIGNED)) AS LAST_ID from telephones');
   ...
   if ($new_id < 10) {
    ...
    $requete = $bdd->prepare('INSERT INTO telephones (ID, Marque, Modele, Mobile, Date_Achat, Duree_Garantie, ID_Utilisateurs) VALUES (:ID, :Marque, :Modele, :Mobile, :Date_Achat, :Duree_Garantie, :ID_Utilisateurs)');
