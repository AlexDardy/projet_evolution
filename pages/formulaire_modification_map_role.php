            ...
            <div class="row">
                <div class="col-lg">
                    <!-- 
                     ...
                     -->
                    <form action="formulaire_modification_map_role.php" method="post">

                        <!-- Input liste déroulante -->
                        <div class="form-group">
                            <label for="id_map_role">Choix de l'utilisateur à modifier</label>
                            <select id="id_map_role" class="form-control" name="id_map_role">                                
                                ...
                                <?php 
                                    ...
                                    $requete = $bdd->prepare('SELECT * FROM map_role');
                                    // ...puis on l'exécute
                                    $requete->execute();
                        ...
                        <button type="submit" class="btn btn-primary">Choisir map_role</button>
                    </form>

                    <hr>

                    <h1>Formulaire de modification map_role</h1>
                    ...
                    <?php 
                        ...
                        /* 
                            ...
                            2) Un article a été choisi et on a donc $_POST['id_map_role'] contenant l'id de l'article, on va pouvoir récupérer l'enregistrement correspondant dans la base de données pour préremplir le formulaire.

                            Pour info, 'id_map_role' correspond à l'attribut "name" de la liste déroulante.
                        */
                        ...
                        */
                        ...
                        /* 
                            Ci-dessous, on va avoir la condition suivante : empty( $_POST['id_map_role'] ) == FALSE
                            
                            Si on la décompose cela donne :
                            $_POST['id_map_role'] correspond à la valeur de l'input "id_map_role" qui a été envoyée par un formulaire au chargement de la page. Si aucun input ne correspond à ce nom, alors $_POST['id_map_role'] sera vide, dans le cas contraire il contiendra la valeur de l'input.
                            ...
                            empty( $_POST['id_map_role] ) signifie donc qu'on va avoir TRUE si $_POST['id_map_role'] est vide ou FALSE s'il contient une valeur.
                            ...
                            Pour résumer, notre condition correspond à la question suivante : "a-t-on une valeur pour un input appelé 'id_map_role' ?"
                        */
                        if( empty($_POST['id_map_role']) == FALSE ){

                            // Si la condition est respectée, on va chercher l'enregistrement correspondant à cet id dans la base de données.
                            
                            // On commence par créer une variable $id_map_role contenant cette valeur
                            $id_map_role = $_POST['id_map_role'];


                            // Préparation de la requête pour récupérer l'enregistrement
                            // Il est important de noter le ":id_map_role", cela correspond à une valeur qu'on va insérer dans la requête par la suite.
                            $requete = $bdd->prepare('SELECT * FROM map_role WHERE ID=:id_map_role');
                            

                            // On exécute la requête en lui passant la variable contenant l'id à chercher. Cette valeur sera insérée à la place de :id_map_role dans la requête
                            /*
                            ...
                            */
                            $requete->execute(['id_map_role' => $id_map_role]);
                    ...
                    <form action="../actions/modification_map_role.php" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="id" name="id" value="<?php echo $id_map_role; ?>" readonly>
                        </div>
                        ...
                        <button type="submit" class="btn btn-primary">Modifier l'map_role</button>
                    </form>
                </div>
            </div>
